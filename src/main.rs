...
impl App {
    ...
    async fn send_transaction(&mut self) -> Result<()> {
        let recipient = Pubkey::from_str(&self.send_state.recipient)
            .context("Invalid recipient address")?;
        
        let amount = self.send_state.amount.parse::<f64>()
...
fn ui(f: &mut Frame, app: &App) {
    ...
    let menu_items = vec!["Home", "Wallet", "Send", "Receive", "Transactions", "Settings"];
...
fn render_send(app: &App) -> Paragraph<'static> {
    ...
    match app.send_state.input_mode {
        ...
        SendInputMode::EditingAmount => {
            ...
            lines.push(Line::from(""));
            lines.push(Line::from(format!("Available balance: {:.9} SOL", app.wallet.balance)));
            lines.push(Line::from(""));
